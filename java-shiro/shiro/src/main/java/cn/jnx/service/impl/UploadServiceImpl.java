...
import cn.jnx.util.file.FileUtil;
...
public class UploadServiceImpl implements UploadService {
    ...
    /**
     * 分块上传 第一步：获取RandomAccessFile,随机访问文件类的对象
     * 第二步：调用RandomAccessFile的getChannel()方法，打开文件通道 FileChannel
     * 第三步：获取当前是第几个分块，计算文件的最后偏移量 第四步：获取当前文件分块的字节数组，用于获取文件字节长度
     * 第五步：使用文件通道FileChannel类的 map（）方法创建直接字节缓冲器 MappedByteBuffer
     * 第六步：将分块的字节数组放入到当前位置的缓冲区内 mappedByteBuffer.put(byte[] b); 第七步：释放缓冲区
     * 第八步：检查文件是否全部完成上传
     * 
     * @param param
     * @return
    ...
    public String chunkUploadByMappedByteBuffer(MultipartFileParam param) throws IOException {
        ...
        String filePath = basePath + "fileUpload" + "/original";
    ...
    public boolean checkUploadStatus(MultipartFileParam param, String fileName, String filePath) throws IOException {
        ...
        // 这一段逻辑有点复杂，看的时候思考了好久，创建conf文件文件长度为总分片数，每上传一个分块即向conf文件中写入一个127，那么没上传的位置就是默认的0,已上传的就是Byte.MAX_VALUE
        // 127
        for (int i = 0; i < completeStatusList.length && isComplete == Byte.MAX_VALUE; i++) {
            // 按位与运算，将&两边的数转为二进制进行比较，有一个为0结果为0，全为1结果为1 eg.3&5 即 0000 0011 & 0000 0101 = 0000
            // 0001 因此，3&5的值得1。
